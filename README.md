...
- **Sign in with what your company already has**: Google or Okta from the environment, or a company's own SAML or OpenID Connect provider registered while the deployment runs and routed by email domain. Any one turns sign-in on; both may be configured at once. Microsoft OAuth is disabled until the owner gate binds immutable Entra tenant and object IDs.
...
configure **Google or Okta** before anybody else can reach the deployment.
...
OPENBOT_OWNER_EMAIL=you@example.com          # optional: admit only this account
```
...
- Microsoft OAuth environment settings stop startup in this owner-only release. Its owner gate must
  bind immutable Entra tenant and object IDs before Microsoft sign-in can be enabled safely.
