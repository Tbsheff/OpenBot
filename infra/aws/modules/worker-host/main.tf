locals {
  providers = toset(["codex", "claude", "grok"])
  auth_devices = {
    codex  = "/dev/sdg"
    claude = "/dev/sdh"
    grok   = "/dev/sdi"
...
resource "aws_ebs_volume" "auth" {
  ...
  availability_zone = var.availability_zone
  size              = var.auth_volume_size_gib
  type              = "gp3"
  encrypted         = true
  kms_key_id        = var.kms_key_arn

  tags = {
    Name     = "${var.name}-${each.key}-auth"
...
resource "aws_instance" "this" {
  ...
  user_data = templatefile("${path.module}/bootstrap.sh.tftpl", {
    ...
    codex_auth_id  = aws_ebs_volume.auth["codex"].id
    claude_auth_id = aws_ebs_volume.auth["claude"].id
    grok_auth_id   = aws_ebs_volume.auth["grok"].id
  })
...
resource "aws_volume_attachment" "auth" {
  ...
  device_name                    = local.auth_devices[each.key]
  instance_id                    = aws_instance.this.id
  volume_id                      = aws_ebs_volume.auth[each.key].id
  stop_instance_before_detaching = true
  skip_destroy                   = true
  # A separately attached EBS volume has delete_on_termination = false.
}
